...
def scan_image(img_file_path):
    ...
    for cnt in contours[1::]:
        ...
        if len(approx) > 6:
            ...
            shapes['Circle'] = [None,pcx,pcy]
            print(len(approx))
        pcx,pcy = cx,cy
    return shapes


def scan_image2(img_file_path):
    
    global shapes
    shapes = {}
    shapes['Circle'] = []
    img = cv2.cvtColor(img_file_path, cv2.COLOR_BGR2GRAY)
    hsvFrame = cv2.cvtColor(img_file_path, cv2.COLOR_BGR2HSV) 
    ...
    res_red = cv2.bitwise_and(img_file_path, img_file_path,  
                              mask = red_mask) 
      
    # For green color 
    green_mask = cv2.dilate(green_mask, kernal) 
    res_green = cv2.bitwise_and(img_file_path, img_file_path, 
                                mask = green_mask) 
      
    # For blue color 
    blue_mask = cv2.dilate(blue_mask, kernal) 
    res_blue = cv2.bitwise_and(img_file_path, img_file_path, 
                               mask = blue_mask) 
