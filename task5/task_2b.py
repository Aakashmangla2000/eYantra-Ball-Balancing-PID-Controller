...
def get_vision_sensor_image():
	...
	return_code, vs = sim.simxGetObjectHandle(client_id, "Vision_sensor", sim.simx_opmode_oneshot_wait)
	# print(return_code,'yo')
	return_code, image_resolution, vision_sensor_image = sim.simxGetVisionSensorImage(client_id, vs,0,sim.simx_opmode_blocking)
	# print(vs,vision_sensor_image,return_code)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	img = img.reshape(image_resolution[1], image_resolution[0],3)	
...
def send_data(client_id, maze_array,table_num):
	...
	##############	ADD YOUR CODE HERE	##############
	# print('hi',len(maze_array))
	# print(client_id)
	for i in range(len(maze_array)):
		inputInts = maze_array[i]
		# inputInts = [1,2,3]
		return_code,retInts,retFloats,retStrings,retBuffer=sim.simxCallScriptFunction(client_id,'top_plate_respondable_t'+str(table_num)+'_1',sim.sim_scripttype_customizationscript,
					'receiveData',inputInts,[],[],'',sim.simx_opmode_blocking)
