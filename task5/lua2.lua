...
function generateHorizontalWalls()
	...
	--*******************************************************
	--               ADD YOUR CODE HERE
	bs = sim.getObjectHandle('force_sensor_pw_t4_1')
    ...
    do
        ...
        do
            ...
            ret = sim.setObjectPosition(ob,-1,{-0.5+(0.1*j),-0.45+(0.1*i),0.700})
...
function generateVerticalWalls()
    ...
    do
        ...
        do
            ...
            ret = sim.setObjectName(ob,st)
            ret = sim.setObjectPosition(ob,-1,{-0.45+(0.1*j),-0.5+(0.1*i),0.700})
            ret = sim.setObjectParent(ob,bs,1)
        end 
    end
	--*******************************************************
...
function deleteWalls()
    ...
    --     end
    -- end
    objectHandle=sim.getObjectHandle('walls_t4_1')
...
function createMaze()
	...
	--               ADD YOUR CODE HERE
    --print(maze_array)
    ob = sim.getObjectHandle(string.format('V_WallSegment_%dx%d@silentError',11,6))
    if (ob ~= -1)
    then
        ret = sim.removeObject(ob)
    end
    ob = sim.getObjectHandle(string.format('V_WallSegment_%dx%d@silentError',1,5))
    if (ob ~= -1)
    then
        ret = sim.removeObject(ob)
    end
    ob = sim.getObjectHandle(string.format('H_WallSegment_%dx%d@silentError',5,11))
    if (ob ~= -1)
    then
        ret = sim.removeObject(ob)
    end
...
function groupWalls()
	...
	sim.setObjectName(shapeHandle,'walls_t4_1')
	--*******************************************************
end
...
function addToCollection()
	--*******************************************************
	--               ADD YOUR CODE HERE
	collectionHandle=sim.getCollectionHandle('colliding_objects_t4')
	objectHandle=sim.getObjectHandle('walls_t4_1')
...
function drawPath(inInts,path,inStrings,inBuffer)
    wallsGroupHandle=sim.getObjectHandle('walls_t4_1')
