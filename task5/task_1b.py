...
def applyPerspectiveTransform(input_img):
	...
	# img2 = cv2.imread(input_img, cv2.IMREAD_COLOR) 
	...
	try: 
		# print(height,width,'hi')
		pts1 = np.float32([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
		pts2 = np.float32([[0, 0], [height, 0], [0, width], [height, width]])
		matrix = cv2.getPerspectiveTransform(pts1, pts2)
		result = cv2.warpPerspective(input_img, matrix, (height, width))

	except Exception as e: 
		# print(e)
		x1,x2,x3,x4,y1,y2,y3,y4 = 0,0,0,0,0,0,0,0
		for i in range(0,len(lmn),2): 
			if lmn[i] > 900 and lmn[i+1] < 30:
				x2 = lmn[i]
				y2 = lmn[i+1]
				# print('2')
			if lmn[i] > 900 and lmn[i+1] > 900:
				x4 = lmn[i]
				y4 = lmn[i+1]
				# print('4')
			if lmn[i] < 30 and lmn[i+1] > 900:
				x3 = lmn[i]
				y3 = lmn[i+1]
				# print('3')
			if lmn[i] < 30 and lmn[i+1] < 30:
				x1 = lmn[i]
				y1 = lmn[i+1]
				# print('1')
		# pts1 = np.float32([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
		# print(lmn)
		pts1 = np.float32([[110, 110], [913, 110], [110, 913], [913, 913]])
		# pts1 = np.float32([[55, 55], [444, 55], [55, 455], [455, 455]])

		# pts1 = np.float32([[22, 21], [1002, 23], [21, 1001], [1000, 1001]])
...
def applyPerspectiveTransform2(input_img):
...
def detectMaze(warped_img):
	...
	##############	ADD YOUR CODE HERE	##############
	# th3 = cv2.adaptiveThreshold(warped_img,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
	...
	matrix = cv2.getPerspectiveTransform(pts1, pts2)
	result = cv2.warpPerspective(warped_img, matrix, (512, 512))
	height, width,_ = result.shape

	
	
	ret,im = cv2.threshold(result,127,255,cv2.THRESH_BINARY)
	kernel = np.ones((5,5),np.uint8)
	erosion = cv2.erode(im,kernel,iterations = 2)
	dilation = cv2.dilate(erosion,kernel,iterations = 1)
	opening = cv2.morphologyEx(im, cv2.MORPH_OPEN, kernel)

	# print(height,width,'hv')
